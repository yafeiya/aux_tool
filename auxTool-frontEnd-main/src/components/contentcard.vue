<template>
  ...
  >
    ...
    <template #title>
      <div style="text-align: center">
        <a @click="modal" v-if="this.pageKind === 'database'">
          ...
          <Modal v-model="modal_intro1" width="1200" style="margin-top: -50px">
          ...
          >
            ...
            >
              <Space style="margin-left: -40px; margin-bottom: 20px">
                <Image
                  :src="this.modelImageUrl"
                  :fit="fit"
                  width="400px"
                  height="220px"
                  :alt="fit"
                  @click.ctrl="uploadModelPNG"
                >
                  <template #error>
        ...
        <a @click="modal_intro1 = true" v-else>
          ...
          <Modal v-model="modal_intro1" title="简介">
            <p
              style="white-space: pre-wrap"
              v-if="this.cancelInfo.Description !== null"
            >
    ...
    <div style="text-align: center">
      ...
      />
      <img
        v-if="this.pageKind == 'defineFunction'"
          ...
          <a @click="editBtn = true">
            <h4 style="margin-top: 130px; color: #054079; height: 10%">
              ...
              >
                ...
                >
                  <FormItem
                    v-for="(item, index) in addFormItemCfg"
                    :label="item.title"
                    :prop="item.name"
                  >
                    ...
                    <div v-if="item.itemType == 'select'">
                      <Select v-model="cardInfo[item.name]">
                        <Option
                          v-for="(option, index2) in item.others"
                          :key="index2"
                          :value="option.value"
                        >
                          {{ option.text }}
...
import {
  ...
  downloadModelFile,
  updateDesignCard,
  updateDefinefunctionCard,
  deleteDefinefunctionCard,
} from "../api/api.js";
import qs from "qs";
export default {
  data() {
  ...
  methods: {
    ...
    outPutXml() {
      ...
      for (var i in this.selections) {
        var temp =
          this.selections[i].Type +
          "/" +
          this.selections[i].Task +
          "/" +
          this.selections[i].Dataset_Id +
          "/" +
          this.selections[i].Table_name;
        csv_path_array.push(temp);
      }
      console.info("ppppppppppp", csv_path_array);
      // let csv_path_array = ["数值数据集/火力分配/波士顿房价数据集0/123.csv","数值数据集/火力分配/波士顿房价数据集0/345.csv"]
      const data = {
        path: csv_path_array + "",
          ...
          for (var i in this.addFormItemCfg) {
            ...
            if (item.isEditOnly == true) {
              ...
              for (var j in this.cardList) {
                if (
                  this.cardInfo.Id != this.cardList[j].Id &&
                  this.cardInfo[item.name] == this.cardList[j][item.name]
                ) {
                  this.$Message["error"]({
                    background: true,
    ...
    // data = ["数值数据集/火力分配/13/loss.csv","数值数据集/火力分配/13/loss.csv"]
    downloadCsv(row) {
    ...
    uploadCard() {
      if (this.viewRange == "private") {
        if (this.cardInfo.released === "11") {
          ...
          var findUrl =
            this.jsonBaseUrl + "/" + this.pageKind + "/" + this.cardInfo.id;
