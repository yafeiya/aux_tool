<template>
  <Tabs class="config" value="1">
    <TabPane label="属性" name="1">
      <Row align="middle">
        <Button class="view" @click="modal = true"
          >预览{{ data.nodeselflabel }}大图</Button
        >
        <Modal
          v-model="modal"
          width="1000"
    ...
    const initchart = (label) => {
      ...
      while (lossDataString[serial3] != null) {
        if (lossDataString[serial3] == '"') {
          serial3++;
          while (
            lossDataString[serial3] != '"' &&
            lossDataString[serial3] != null
          ) {
      ...
      if (nodelabel == "动作分布") {
        ...
        // console.log("执行动作分布")
      } else if (nodelabel == "奖励曲线") {
        myChart.setOption(option2);
        miniChart.setOption(option2);
        // console.log("执行奖励曲线")
      } else if (nodelabel == "损失函数") {
