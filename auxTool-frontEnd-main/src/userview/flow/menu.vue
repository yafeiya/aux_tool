  ...
  >
    ...
    <Space>
      ...
      <Modal v-model="editMenu" width="400" style="margin-top: 0px">
        ...
        >
          ...
          <FormItem label="三级菜单" prop="grandsonMenu">
            <!-- 修正这里 -->
            <Input
              v-model="menuform.grandsonMenu"
              placeholder="输入三级菜单名称"
              style="width: 250px"
            ></Input>
          </FormItem>
        </Form>
    ...
    <Submenu :name="item.name" v-for="item in hasChildren(list)">
      ...
      <Submenu
        ...
        @mousedown.right="
          startDrag($event, item.title, subitem.title, subitem.title)
        "
      >
        <template #title>
          <span>{{ subitem.title }}</span>
        ...
        <MenuItem
          ...
          @mousedown="
            startDrag($event, item.title, subitem.title, lastitem.title)
          "
        >
...
export default defineComponent({
  ...
  async setup() {
    ...
    const item_data = menu.find((menuItem) => menuItem.title === "数据准备");
    if (item_data) {
    ...
    const item_model = menu.find((menuItem) => menuItem.title === "模型选择");
    if (item_model) {
      item_model.children.push(...modelbase);
    }

    let defineFunction = (await getMenuInfo("defineFunction")).data;
    const item_train = menu.find((menuItem) => menuItem.title === "训练监控");
    if (item_train) {
      item_train.children.push(...defineFunction[0].children);
    }
    const item_eval = menu.find((menuItem) => menuItem.title === "模型评估");
    if (item_eval) {
