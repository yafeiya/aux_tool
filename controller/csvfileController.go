...
// 获取csv文件属性
func GetCSVInfo(string csv_path)(int, int, []string) {
	// 打开CSV文件
...
// 添加csv表
func CreateTable(string Task, string Type, string Dataset_name, int numColumns, int numRows, []string Types, string csv_name) {
	db := common.InitDB()
	Types := ""
	...
	// 判重处理
	db.Where("Task = ? and Type = ? and Dataset_name = ? and Table_name = ?", task, Type, dataset_name, Table_name).First(&datatable)
	if database.Id != 0 {
		fmt.Println("该卡片已存在")
